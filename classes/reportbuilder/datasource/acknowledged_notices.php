...
/**
 * Acknowledged notices datasource for Report Builder.
 *
 * Shows only rows from local_awareness_ack where action = ACTION_ACKNOWLEDGED.
...
class acknowledged_notices extends datasource {
    ...
    public static function get_name(): string {
        return get_string('datasource:acknowledgednotices', 'local_awareness');
    }
    ...
    protected function initialise(): void {
        ...
        // Restrict to acknowledged rows only.
        $actionparam = database::generate_param_name();
        $this->add_base_condition_sql(
            "{$ackalias}.action = :{$actionparam}",
            [$actionparam => acknowledgement_persistent::ACTION_ACKNOWLEDGED]
        );
