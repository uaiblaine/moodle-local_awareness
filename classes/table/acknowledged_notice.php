...
class acknowledged_notice extends table_sql implements renderable {
    ...
    public function __construct(
        ...
        // Set download status.
        $currenttime = userdate(time(), get_string('report:timeformat:sortable', 'local_awareness'), null, false);
        $this->is_downloading($download, get_string('report:acknowledged', 'local_awareness', $currenttime));
    ...
    protected function define_table_columns() {
        if ($this->is_downloading()) {
            $cols = [
                'noticetitle' => get_string('notice:title', 'local_awareness'),
            ...
            $cols['timecreated'] = get_string('report:timecreated_server', 'local_awareness');
            $cols['timecreated_spreadsheet'] = get_string('report:timecreated_spreadsheet', 'local_awareness');
        } else {
            $cols = [
                'noticetitle' => get_string('notice:title', 'local_awareness'),
                'username' => get_string('username'),
                ...
                'lastname' => get_string('lastname'),
                'idnumber' => get_string('idnumber'),
                'hlinkcount' => get_string('notice:hlinkcount', 'local_awareness'),
                'timecreated' => get_string('report:timecreated_server', 'local_awareness'),
            ];
    ...
    protected function get_sql_and_params(bool $count = false) {
        ...
        $sql = "SELECT {$select}
                  FROM {local_awareness_ack} {$alias}
                 WHERE {$where}";
    ...
    protected function col_timecreated(\stdClass $row) {
        ...
        if ($row->timecreated) {
            return userdate($row->timecreated, get_string('report:timeformat:sortable', 'local_awareness'), null, false);
        } else {
            return '-';
        }
    }
