...
use local_awareness\form\active_filter_form;
use local_awareness\form\add_filter_form;
...
class report_filter {
    ...
    /**
     * Filter form.
     * @var \local_awareness\form\add_filter_form
     */
    private $addfilterform;

    /**
     * Active filter form.
     * @var \local_awareness\form\active_filter_form
     */
     ...
     * Constructor.
     *
     * @param string|\moodle_url $baseurl Base URL.
     * @param string $tablealias Table alias.
     *
     * @throws \coding_exception
     */
    public function __construct(string|\moodle_url $baseurl, string $tablealias = '') {
        $tablealias = $tablealias ? $tablealias . "." : '';
        $this->filterfields = [
            "timecreated" => new \user_filter_date('time', get_string('time'), false, "{$tablealias}timecreated"),
        ];
        // Ensure baseurl is always a string for internal use.
        $this->baseurl = ($baseurl instanceof \moodle_url) ? $baseurl->out(false) : $baseurl;
        ...
        if (isset($SESSION->noticereportfilter)) {
            ...
            foreach ($filters as $fname => $data) {
                if (isset($this->filterfields[$fname])) {
                    [$s, $p] = $this->filterfields[$fname]->get_sql_filter(reset($data));
                    $filtersql[] = $s;
                    $params = $params + $p;
                }
            }
