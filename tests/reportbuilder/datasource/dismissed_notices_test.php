...
/**
 * Unit tests for the dismissed_notices datasource.
 *
 ...
 */
final class dismissed_notices_test extends core_reportbuilder_testcase {
    /**
    ...
    public function test_datasource_default(): void {
        ...
        // One dismissed, one acknowledged — only dismissed should appear.
        $this->create_ack($noticeid, $user1->id, acknowledgement_persistent::ACTION_DISMISSED);
        $this->create_ack($noticeid, $user2->id, acknowledgement_persistent::ACTION_ACKNOWLEDGED);

        /** @var core_reportbuilder_generator $generator */
        $generator = $this->getDataGenerator()->get_plugin_generator('core_reportbuilder');
        $report    = $generator->create_report([
            'name'    => 'Dismissed notices test',
    ...
    public function test_datasource_columns(): void {
        ...
        $noticeid = $this->create_notice('Dismissed Column Test');
        $this->create_ack($noticeid, $user->id, acknowledgement_persistent::ACTION_DISMISSED);

        /** @var core_reportbuilder_generator $generator */
        $generator = $this->getDataGenerator()->get_plugin_generator('core_reportbuilder');
        $report    = $generator->create_report([
            'name'    => 'Dismissed columns test',
            'source'  => dismissed_notices::class,
            'default' => 0,
        ]);
        $generator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'notice:title']);
        $generator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'acknowledgement:username']);

        $content = $this->get_custom_report_content($report->get('id'));
        $this->assertCount(1, $content);
        $row = array_values(reset($content));
        $this->assertEquals('Dismissed Column Test', $row[0]);
    }
    ...
    public function test_datasource_filters(): void {
        ...
        $notice1 = $this->create_notice('Dismissed Filter A');
        $notice2 = $this->create_notice('Dismissed Filter B');

        $this->create_ack($notice1, $user1->id, acknowledgement_persistent::ACTION_DISMISSED);
        $this->create_ack($notice2, $user2->id, acknowledgement_persistent::ACTION_DISMISSED);

        /** @var core_reportbuilder_generator $generator */
        $generator = $this->getDataGenerator()->get_plugin_generator('core_reportbuilder');
        $report    = $generator->create_report([
            'name'    => 'Dismissed filter test',
            'source'  => dismissed_notices::class,
            'default' => 0,
        ]);
        $generator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'notice:title']);
        ...
        $content = $this->get_custom_report_content($report->get('id'), 0, [
            ...
            'notice:title_value'    => 'Dismissed Filter B',
        ]);
        $this->assertCount(1, $content);
        $row = array_values(reset($content));
    ...
    public function test_stress_datasource(): void {
        ...
        $user     = $this->getDataGenerator()->create_user();
        $noticeid = $this->create_notice('Stress Dismissed');
        $this->create_ack($noticeid, $user->id, acknowledgement_persistent::ACTION_DISMISSED);

        $this->datasource_stress_test_columns(dismissed_notices::class);
        $this->datasource_stress_test_columns_aggregation(dismissed_notices::class);
        $this->datasource_stress_test_conditions(dismissed_notices::class, 'notice:title');
    }
}
