...
// Set up page.
$thispage = new moodle_url('/local/awareness/report/dismissed_report.php', ['noticeid' => $noticeid]);
...
if (empty($notice)) {
    echo $output->header();
    echo $OUTPUT->notification(get_string('notification:nodis', 'local_awareness'), 'notifyinfo');
    echo $output->footer();
} else {
    ...
    $table = new dismissed_notice(
        'dismissed_notice_table',
        $thispage,
        $notice->id,
    ...
    echo $output->heading(get_string('report:dismissed_desc', 'local_awareness'), 4);
